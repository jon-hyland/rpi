...
namespace Rpi.Gpio.Helpers
{
    public static class ShellHelper
    {
        ...
        {
            return BashAsync(command, timeoutMs, writeLog).Result;
        }

        /// <summary>
        /// Runs any string as a Linux bash command.
        /// </summary>
        private static async Task<string> BashAsync(string command, int? timeoutMs, bool writeLog)
        {
            StringBuilder sb = new StringBuilder();
            try
            {
                if (writeLog)
                    Log.Instance.WriteMessage("Bash", $"Running process: {command}");
                ...
                if (writeLog)
                {
                    Log.Instance.WriteMessage("Bash", $"Process exited with code {exitCode}");
                    Log.Instance.WriteMessage("Bash", $"Process response: {sb}");
                }
            }
